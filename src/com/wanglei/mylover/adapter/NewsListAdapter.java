...
public class NewsListAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		View vi = convertView;
		if (vi == null) {
			vi = inflater.inflate(R.layout.tushuoitem, null);
			viewHolder = new ViewHolder();
			viewHolder.item_des = (TextView) vi.findViewById(R.id.item_des); // 摘要
			viewHolder.item_img = (ImageView) vi.findViewById(R.id.item_image); // 缩略图路径
			viewHolder.item_guanzhu = (ImageView) vi.findViewById(R.id.guanzhu);
		...
		viewHolder.item_des.setText((String) map.get("newslist_item_digest"));// 摘要
		imageLoader.DisplayImage(URLS[position], viewHolder.item_img, false);

		if (isCheckMap != null && isCheckMap.containsKey(position)) {
			if (isCheckMap.get(position)) {
				viewHolder.item_guanzhu
						.setBackgroundResource(R.drawable.guanzhu_pressed);
			} else {
				viewHolder.item_guanzhu
						.setBackgroundResource(R.drawable.guanzhu);
			}
		} else {
			viewHolder.item_guanzhu.setBackgroundResource(R.drawable.guanzhu);
		}
		viewHolder.item_guanzhu.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (isCheckMap != null && isCheckMap.containsKey(position)) {
					if (isCheckMap.get(position)) {
						...
						viewHolder.item_guanzhu
								.setBackgroundResource(R.drawable.guanzhu_pressed);
						Toast.makeText(context, "恭喜您关注成功", 0).show();
					}
				} else {
					isCheckMap.put(position, true);
					viewHolder.item_guanzhu
